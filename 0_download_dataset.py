...
def main():
    ...
    for dataname in urls:
        raw_dir = Path('data', 'raw', dataname)
        ...
        for filepath in raw_dir.glob('*.txt'):
            with open(str(filepath)) as f:
                ...
                labeled_train_dir = raw_dir.joinpath('labeled', 'train')
                labeled_train_dir.mkdir(parents=True, exist_ok=True)
                labeled_test_dir = raw_dir.joinpath('labeled', 'test')
    ...
    with open(str(nyc_taxi_raw_path), 'r') as f:
        for i, line in enumerate(f):
            ...
            labeled_data.append(tokens)
    nyc_taxi_train_path = raw_dir.joinpath('labeled', 'train', nyc_taxi_raw_path.name).with_suffix('.pkl')
    ...
    nyc_taxi_test_path = raw_dir.joinpath('labeled', 'test', nyc_taxi_raw_path.name).with_suffix('.pkl')
